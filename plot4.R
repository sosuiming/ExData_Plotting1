...
plot(dtimes, df$v, type='l', xlab='datetime', ylab='Voltage');
plot(c(dtimes,dtimes,dtimes), c(df$sm1,df$sm2,df$sm3), type='l', xlab='', ylab='Energy sub metering');
plot(dtimes, df$grp, type='l', xlab='datetime', ylab='Global_react_power)');
dev.off()



xyplot(dtimes, c(gap, v, c(df$sm1,df$sm2,df$sm3), df$gi));
